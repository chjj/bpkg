...
class Module {
  ...
  async ExportAllDeclaration(node, code, id) {
    ...
    for (const key of exports) {
      ...
      out += `exports[${string(key)}] = ${name}[${string(key)}];\n`;
  ...
  async ExportNamedDeclaration(node, code, _id) {
        ...
        case 'VariableDeclaration': {
          ...
          for (const {id, init} of decl.declarations) {
            ...
            if (id.type === 'ObjectPattern') {
              const name = `__bpkg_declare_${_id}_${tmp++}__`;
              ...
              for (const {key, value} of id.properties) {
                ...
                declarations.push([value.name, `${name}[${string(key.name)}]`]);
              }
    ...
    if (source) {
      ...
      for (const [value, key] of exports) {
        if (value === 'default') {
          out += `exports[${string(key)}] = ${name}.__esModule\n`
               + `  ? ${name}['default']\n`
               + `  : ${name};\n`;
