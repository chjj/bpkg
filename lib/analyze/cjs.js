...
function visitor(node, state) {
  ...
  // const foo = bar.baz;
  // const foo = bar['baz'];
  if (isAccessRequire(node, state.imports)) {
    const decl = node.declarations[0];
    const name = decl.id.name;
    const source = state.imports.get(decl.init.object.name)[1];
    const imported = getPropertyName(decl.init.property);
...
function setExport(state, key, node) {
  ...
  } else if (isRequireRef(node, state.imports)) {
    const source = state.imports.get(node.object.name)[1];
    const imported = getPropertyName(node.property);
...
function isAccessRequire(node, imports) {
  // const foo = bar.baz;
  // const foo = bar['baz'];
  if (node.type !== 'VariableDeclaration')
    return false;
  ...
  return isRequireRef(decl.init, imports);
}
...
function isRequireRef(node, imports) {
  // foo.bar
  // foo['bar']
  if (node.type !== 'MemberExpression')
    return false;
  ...
  const desc = imports.get(object.name);
...
exports.isMultiRequire = isMultiRequire;
exports.isRequire = isRequire;
exports.isRequireDot = isRequireDot;
