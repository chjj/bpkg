...
class Bundle {
  ...
  async getModule(path) {
    ...
    if (!this.moduleMap.has(path)) {
      ...
      if (this.target == null) {
        if (!this.multi)
          this.target = module.sourceType() === 'module' ? 'esm' : 'cjs';
        else
          this.target = 'esm';
      }
    ...
    for (let item of this.plugins) {
      ...
      if (typeof func === 'string') {
        if (plugins[func])
          ...
          func = require(await this.resolver.resolve(func));
      }
  ...
  async redirect(location, from, isImport) {
    ...
    for (const plugin of this.running) {
      ...
      location = await plugin.redirect(location, from, isImport);
  ...
  async bundle(input, output) {
    ...
    input = await pkg.resolver.resolve(input);
  ...
  async _bundle(input, output) {
    ...
    for (const name of this.requires) {
      const path = await this.resolver.resolve(name);
    ...
    if (this.env === 'node'
      ...
      if (module.format === 'module') {
        ...
        out += '(function(__exports) {';
        out += '\n';
        out += module.code;
        out += '\n';
        out += '}).call(void 0, exports);';
      } else {
        out += '\n';
        out += module.code;
      }
