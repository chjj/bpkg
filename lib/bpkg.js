...
class Bundler {
  ...
  module(parent, path, location) {
    assert(!parent || parent instanceof Module);
    assert(typeof path === 'string');
    assert(!location || typeof location === 'string');
  ...
  bundle(file, output) {
    ...
    if (this.env === 'browser') {
      ...
      if (this.hasProcess)
        process = this.module(root, builtins._process, 'process').id;
    ...
    if (output) {
      if (this.collectBindings) {
        ...
        for (const binding of this.bindings)
          fs.copyFileSync(binding, resolve(output, basename(binding)));

        fs.writeFileSync(resolve(output, 'index.js'), out);
      } else {
