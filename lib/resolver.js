...
class Resolver {
  ...
  async esmFileFormat(file) {
    ...
    if (this.stripTypes && withinNodeModules(file)) {
      switch (ext) {
        case '.ts':
        case '.cts':
        case '.mts':
    ...
    if (ext === '.js' || (this.stripTypes && ext === '.ts')) {
      ...
      if (this.defaultType != null) {
        if (this.defaultType === 'module' && withinNodeModules(file))
          return 'commonjs';
  ...
  async detectModuleSyntaxOld(path) {
...
function withinNodeModules(file) {
  assert(typeof file === 'string');
  assert(isAbsolute(file));
