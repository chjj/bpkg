  ...
  async _bundle(src, dest) {
    ...
    if (this.env === 'node'
      ...
      for (let module of this.modules) {
        const {id, location, path, pkg} = module;

        let {code} = module;

        code = await pkg.transform(path, code);
...
class Package {
  constructor(bundler, root) {
    ...
    this.deps = [];
    this.license = null;
    this.resolve = createResolver(this.path);
  }
  ...
  init() {
    ...
    pkg.browserify = parseConfig(pkg.browserify, null);
    pkg.bpkg = parseConfig(pkg.bpkg, this.bundler.env);

    if (this.bundler.env === 'browser'
        && pkg.browserify.transform.length > 0) {
      pkg.bpkg.transform = pkg.browserify.transform;
    }
    ...
    this.transforms = pkg.bpkg.transform;
    this.deps = deps;
  ...
  async _copy(dest) {
    ...
    for (const [file, stat] of this.traverse()) {
      ...
      code = await this.transform(from, code);
  ...
  async _transform(file, code, transform) {
    ...
    const stream = require(path)(file, options, code);
...
class Module {
  ...
  ExportAllDeclaration(node, code, id) {
    // export * from 'foo';
  ...
  ExportDefaultDeclaration(node, code, id) {
    ...
    if (this.bundler.requireCompat) {
      out += 'if ((exports.default && typeof exports.default === "object")\n';
      out += '    || typeof exports.default === "function") {\n';

      if (id === 0) {
        out += '  module.exports = exports = exports.default;\n';
        ...
        out += `      exports.default[${k}] = exports[${k}];\n`;
        out += '  }\n';
        out += '  ({ __proto__: exports.default });\n';
        out += '  module.exports = exports = exports.default;\n';
      }
...
function transpile(code, target) {
  assert(typeof code === 'string');
  assert(typeof target === 'string');

  const n = (Math.random() * 0x100000000) >>> 0;
  const tmpIn = resolve(PREFIX, `in-${n}.js`);
  const tmpOut = resolve(PREFIX, `out-${n}.js`);

  if (!isDirectory(PREFIX))
    fs.mkdirSync(PREFIX, 0o755);
  ...
  try {
    tsc = resolveGlobal('typescript/bin/tsc');
  } catch (e) {
    ;
  }

  // Use the typescript compiler to output es3.
  try {
    exec(PREFIX, 'pipe', [process.argv[0],
                          tsc, '--allowJs',
                          '-m', 'none',
                          '--target', target,
                          '--outFile', tmpOut,
...
function resolveGlobal(location) {
  assert(typeof location === 'string');

  const paths = module.paths.slice();

  try {
    module.paths.push(...globalDirs);
...
function parseConfig(config, env) {
  assert(env == null || typeof env === 'string');
  ...
  if (env && (config.node || config.browser))
    config = config[env] || Object.create(null);
  ...
  if (!Array.isArray(config.transform))
    throw new Error('Invalid config.transform.');

  const {transform} = config;

  for (let i = 0; i < transform.length; i++) {
    let file = transform[i];
