...
class Bundle {
  ...
  async redirect(location) {
    ...
    for (const plugin of this.running) {
      ...
      location = await plugin.redirect(location);
...
class Package {
  ...
  async _replicate(dest, optional) {
    ...
    await this.traverse(async (file, stat) => {
      ...
      if (ext === '' && stat.size < (20 << 20))
        isCode = hasHashbang(await fs.readFile(from));
...
class Module {
  ...
  makeRequire(id, location) {
    ...
    if (this.bundle.env === 'browser')
      return `__browser_require__(/* ${string(location)} */ ${id}, module)`;

    return `__node_require__(/* ${string(location)} */ ${id})`;
  }
  ...
  async tryBinding(location) {
    ...
    this.bundle.log(`Rebuilding ${basename(this.root)}.`);
...
function hasHashbang(raw) {
  ...
  // Limit at 256 bytes so we don't end up
  // parsing a 100mb file of god-knows-what.
  let i = 2;

  for (; i < raw.length && i < 256; i++) {
    const ch = raw[i];
