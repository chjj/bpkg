...
class Linker {
  ...
  makeRawRequire(path, isImport) {
    assert(typeof path === 'string');
    assert(typeof isImport === 'boolean');

    if (isImport && this.bundle.target !== 'esm') {
      this.warning('cannot import module: %s.', path);
      return this.makeError(path);
    }

    if (isImport)
      return `(await import(${string(path)}))`;

    if (this.bundle.env === 'browser')
      return `__browser_error__(${string(path)})`;
  ...
  async require(location, isImport) {
    ...
    if (this.bundle.multi) {
      ...
      if (isImport && !this.resolver.isCore(location)) {
        const req = await this.resolver.resolve(location, false);
        const imp = await this.resolver.resolve(location, true);

        if (req !== imp)
          throw new Error(`Cannot import ${location} with require.`);

        const fmt = await this.resolver.format(req);

        if (fmt === 'module' || fmt === 'wasm' || fmt === 'unknown')
          throw new Error(`Cannot import ${location} with require.`);
      }
