      ...
      case 'VariableDeclaration':
        if (node.kind !== 'var') {
          for (const {id} of node.declarations) {
            if (hasScopeCollision(id))
              return true;
          }
        }
        break;
      case 'ClassDeclaration':
        if (hasScopeCollision(node.id))
          return true;
        break;
    }
  }
...
function hasScopeCollision(node) {
  if (!node)
    return false;

  if (node.type === 'Identifier') {
    return CJS_SCOPE.includes(node.name);
  } else if (node.type === 'ObjectPattern') {
    for (const prop of node.properties) {
      if (hasScopeCollision(prop))
        return true;
    }
  } else if (node.type === 'ArrayPattern') {
    for (const el of node.elements) {
      if (hasScopeCollision(el))
        return true;
    }
  } else if (node.type === 'Property') {
    return hasScopeCollision(node.value);
  } else if (node.type === 'AssignmentPattern') {
    return hasScopeCollision(node.left);
  } else if (node.type === 'RestElement') {
    return hasScopeCollision(node.argument);
  }
