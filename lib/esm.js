...
class ESM {
  ...
  async hasTopLevelAwait(root) {
    ...
    for (const source of exprs) {
      if (await this.isRealImport(source, true))
        return true;
    }
  ...
  async analyzeTopLevelAwait(root, sources) {
    ...
    for (const source of imports) {
      ...
      try {
        path = await this.resolver.resolve(source, true);
      } catch (e) {
      ...
      if (!path)
        return this.bundle.ignoreMissing;
  ...
  async transform(root, code) {
    ...
    if (this.bundle.target !== 'esm') {
      if (await this.hasTopLevelAwait(root)) {
    ...
    for (const {type, imported, local} of node.specifiers) {
      switch (type) {
        case 'ImportNamespaceSpecifier': {
          ...
          if (!this.bundle.loose) {
            this.scope.vars.set(local.name, 'namespace');
            this.scope.imports.set(local.name, name);
          }
