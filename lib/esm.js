...
class ESM {
  ...
  async getExports(specifier) {
    ...
    if (this.bundle.env === 'browser') {
      const file = builtins[path];

      if (!file)
        return new Map();

      return cjs.getExports(file);
    }
  ...
  async analyze(root) {
    ...
    for (const [name, desc] of exports) {
      ...
      const path = await this.module.tryResolve(source, isImport);
    ...
    for (const source of reexports) {
      if (!await this.module.tryResolve(source, isImport))
        continue;
  ...
  async transform(root, code) {
    ...
    if (this.module.format !== 'module') {
      if (this.bundle.target === 'esm' || this.bundle.shake) {
        return this.module._replace(root, code, this, {
          Literal: this.bundle.target === 'esm' ? this.Literal : null,
          CallExpression: this.ResolveRequires,
          MemberExpression: this.ResolveRequires,
          VariableDeclaration: this.ResolveRequires
        });
      }
      return code;
    }
  ...
  async ResolveRequires(node) {
    if (!this.bundle.shake)
      return null;

    // require('foo')
    if (cjs.isRequire(node)) {
      const source = node.arguments[0].value;
      const [name, path] = await this.resolveSymbol('default', source, false);

      if (path === source)
        return null;

      const expr = `require(${string(path)})`;

      if (name === 'default')
        return [node, expr];

      return [node, `${expr}[${string(name)}]`];
    }

    // require('bar').baz
    // require('bar')['baz']
    if (cjs.isRequireDot(node)) {
      const source = node.object.arguments[0].value;
      const imported = cjs.getPropertyName(node.property);
      const [name, path] = await this.resolveSymbol(imported, source, false);
    ...
    if (cjs.isMultiRequire(node)) {
      ...
      for (const {key, value} of pattern.properties) {
        const imported = cjs.getPropertyName(key);
      ...
      if (sources.size === 1 && sources.has(source))
        return null;

      let out = '';

      for (const [source, imports] of sources) {
        const expr = `require(${string(source)})`;

        let ident = expr;

        if (imports.length > 1) {
          ident = this.makeTempName('require');
          out += this.makeVar(ident, expr);
        }

        for (const [imported, local] of imports) {
          if (imported === 'default')
            out += this.makeVar(local, ident);
          else
            out += this.makeVar(local, `${ident}[${string(imported)}]`);
        }
      }
