...
class Bundler {
  ...
  async pluginInit(src, dest) {
    ...
    try {
      root = await findRoot(src);
      resolve = this.resolver(root);
    } catch (e) {
      err = e;
    }
    ...
    for (let item of this.plugins) {
      ...
      if (typeof func === 'string') {
        ...
        func = require(await resolve(func));
      }
...
class Package {
  constructor(bundler, root) {
    ...
    this.deps = [];
    this.license = null;
    this.resolve = bundler.resolver(this.path);
  }
  ...
  async init() {
    ...
    const normalize = async (location) => {
      ...
      return './' + relative(root, await this.resolve(location));
    };
  ...
  async _copy(dest) {
    ...
    for (const [file, stat] of (await this.traverse())) {
      ...
      if (this.bundler.extensions.indexOf(ext) !== -1) {
  ...
  async requirify() {
    ...
    if (pkg.isBrowser) {
      if (Object.keys(pkg.browser).length > 0) {
        ...
        try {
          full = await this.pkg.resolve(this.path);
        } catch (e) {
          full = null;
        }
    ...
    const pathResolve = this.bundler.resolver(path);

    return async (location) => {
      const path = await pathResolve(location);
