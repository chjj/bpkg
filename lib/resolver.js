...
class Resolver {
  ...
  async esmFileFormat(file) {
    ...
    if (this.stripTypes && file.includes('/node_modules/')) {
      switch (ext) {
        case '.ts':
        case '.cts':
        case '.mts':
    ...
    if (ext === '.js' || (this.stripTypes && ext === '.ts')) {
      ...
      if (this.defaultType != null) {
        if (this.defaultType === 'module' && file.includes('/node_modules/'))
          return 'commonjs';
  ...
  async detectModuleSyntax(path) {
    ...
    // --experimental-strip-types
    if (this.stripTypes) {
      if (extname(path) === '.ts' && !path.includes('/node_modules/'))
        code = tsParse(code);
    }
...
function tsParse(code) {
  if (!amaro)
    amaro = require('amaro/dist/index').transformSync;
  return amaro;
}
