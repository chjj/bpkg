...
class Resolver {
  ...
  async directory(specifier, isImport = false) {
  ...
  async requirable(specifier) {
    ...
    try {
      reqPath = await this.resolve(specifier, false);
      impPath = await this.resolve(specifier, true);
    } catch (e) {
      return false;
    }
    ...
    const fmt = await this.format(reqPath);
