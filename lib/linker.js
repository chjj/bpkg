...
const {ModuleError} = require('./resolver');
...
class Linker {
  ...
  makeRequire(module, specifier, flag) {
    ...
    if (flag === importTypes.REQUIRE) {
      if (module.isAsync())
        throw new ModuleError('ERR_REQUIRE_ASYNC_MODULE', specifier);
  ...
  async requireBuiltin(specifier) {
    assert(typeof specifier === 'string');
    assert(this.bundle.env === 'browser');

    const flag = importTypes.REQUIRE;

    if (!this.resolver.isCore(specifier))
      return this.makeError(specifier, flag);

    if (this.resolver.isExternal(specifier, false)) {
      this.warning('ignoring external module: %s.', specifier);
      return this.makeError(specifier, flag);
    }

    const id = await this.module.tryResolve(specifier, false);

    assert(id != null);
    assert(!isAbsolute(id));

    const path = builtins[id];
  ...
  isGetBuiltin(node, code) {
    // e.g.
    // process.getBuiltinModule('foo')
    if (node.type !== 'CallExpression')
      return false;

    if (!isAccess(node.callee, 'process', 'getBuiltinModule'))
      return false;
  ...
  async CallExpression(node, code) {
    ...
    if (this.bundle.env === 'browser' && this.isGetBuiltin(node, code)) {
      const arg = getString(node.arguments[0]);
      return [node, await this.requireBuiltin(arg)];
    }
