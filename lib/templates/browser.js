...
__PREFIX__ (async function(global) {
...
async function __browser_require__(id, parent) {
...
// endif
  ...
  try {
    if (esm) {
      await func.call(void 0, _exports, meta, _module);
    } else {
      await func.call(_exports, _exports, _require,
                      _module, filename, dirname, _module);
    }
  } catch (e) {
...
async function __browser_compile_esm__(name, raw, imports) {
  if (typeof name !== 'string' ||
      ...
      imports === null) {
    __browser_error__(name);
    return null;
  }

  if (typeof WebAssembly !== 'function' || typeof fetch !== 'function')
...
// if __TIMERS__
;(async function() {
  var timers = await __browser_require__(__TIMERS__, null);
...
// if __MICROTASK__
queueMicrotask = await __browser_require__(__MICROTASK__, null);
// endif

// if __PROCESS__
process = await __browser_require__(__PROCESS__, null);
// endif

// if __BUFFER__
Buffer = (await __browser_require__(__BUFFER__, null)).Buffer;
// endif
...
;(async function() {
  ...
  for (var i = 0; i < requires.length; i++)
    await __browser_require__(requires[i], null);
})();
// endif

// if !__ESM__
var __browser_main__ = __REQUIRE0__;
// endif
