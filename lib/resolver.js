...
class Resolver {
  ...
  async aliasFile(x, dir) {
    ...
    if (!isAbsolute(shim))
      throw new ModuleError('ERR_INVALID_PACKAGE_SHIM', this.field, shim);
  ...
  // 10. for each key KEY in SHIMS and each extention EXT in EXTENSIONS:
  ...
  async getShims(dir) {
    ...
    if (isString(replacements) && isString(main)) {
      ...
      shims[key] = safeResolve(scope, replacements, this.field);
    } else if (isObject(replacements)) {
      for (const k of Object.keys(replacements)) {
        const key = maybeResolve(scope, k, this.field);
        const val = replacements[k];

        if (val === false)
          shims[key] = EMPTY;
        else if (isString(val))
          shims[key] = maybeResolve(scope, val, this.field);
      }
    }
...
const errors = {
  ...
  ERR_INVALID_PACKAGE_SHIM: 'Invalid %s shim \'%s\''
};
