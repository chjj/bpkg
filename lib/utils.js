...
function isAccess(node, object, property) {
  ...
  if (node.computed) {
    if (getString(node.property) !== property)
      return false;
  } else {
...
function getTypeAttr(node) {
  ...
  for (let i = node.attributes.length - 1; i >= 0; i--) {
    ...
    if (getIdent(attr.key) === 'type')
      return getString(attr.value);

    if (isStringLiteral(attr.key) && attr.key.value === 'type')
      return getString(attr.value);
  }
...
function pluckProp(node, name) {
  ...
  for (let i = node.properties.length - 1; i >= 0; i--) {
    ...
    if (!prop.computed && getIdent(prop.key) === name)
      return prop.value;

    if (getString(prop.key) === name)
      return prop.value;
  }
...
function getTypeProp(node) {
  const obj = pluckProp(node, 'with');

  if (!obj)
    return null;

  const prop = pluckProp(obj, 'type');
...
exports.getTypeAttr = getTypeAttr;
exports.getTypeProp = getTypeProp;
