...
class Bundler {
  ...
  init(options) {
    ...
    if (typeof this.browserField === 'boolean')
      this.browserField = options.browserField;

    if (typeof this.ignoreMissing === 'boolean')
      this.ignoreMissing = options.ignoreMissing;

    if (typeof this.collectBindings === 'boolean')
      this.collectBindings = options.collectBindings;

    if (typeof this.excludeSource === 'boolean')
      this.excludeSource = options.excludeSource;

    if (typeof this.noLicense === 'boolean')
  ...
  async transform(code) {
    ...
    for (const transform of this.transforms)
      code = await this._transform(code, transform);

    return code;
  }

  async _transform(code, transform) {
    assert(typeof code === 'string');
    ...
    const file = resolve(this.rootPkg.root, '_stream_0.js');
...
class Package {
  ...
  async _copy(dest) {
    ...
    for (const [file, stat] of this.traverse()) {
      ...
      code = await this.bundler.transform(code);
