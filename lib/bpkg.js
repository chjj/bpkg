...
class Bundle {
  ...
  async rewrite(path) {
    ...
    for (const plugin of this.running) {
      ...
      path = await plugin.rewrite(path);
  ...
  async final(code, module = null) {
    ...
    for (const plugin of this.running) {
      ...
      code = await plugin.final(code, module);
...
class Package {
  ...
  async _replicate(dest, optional) {
    ...
    for (const [file, stat] of (await this.traverse())) {
      ...
      code = await this.bundle.final(code, module);

      if (this.bundle.minify)
        code = minify(code);

      const outFile = await this.bundle.rewrite(to);
