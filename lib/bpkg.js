...
class Bundle {
  ...
  async _build() {
    ...
    if (this.env === 'node'
        ...
        && requires.length === 0
        && !this.hasMeta
        && (!this.esm || (module.exports.size === 0 && !module.hasExports))) {
      if (module.hashbang)
        out += module.hashbang + '\n';
      ...
      if (this.env === 'browser') {
        out = await template('browser.js', {
          ...
          mark: this.needsMark,
          declare: this.needsDeclare,
          proxy: this.needsProxy,
          timers,
          microtask,
          process,
          buffer,
...
class Module {
  ...
  async ImportDeclaration(node, code, id) {
    ...
    if (default_) {
      out += `var ${default_} = ${name}.__esModule\n`
           + `  ? ${name}['default']\n`
           + `  : ${name};\n`;
    }
  ...
  async ExportAllDeclaration(node, code, id) {
    ...
    if (id === 0) {
      out += '__bpkg_mark__(exports);\n';
      this.bundle.needsMark = true;
    }
  ...
  async ExportDefaultDeclaration(node, code, _id) {
    ...
    if (exposed && id && id.name) {
      ...
      out += '__bpkg_declare__(exports, \'default\', function() {\n';
      out += `  return ${id.name};\n`;
      out += '});\n';
      this.bundle.needsDeclare = true;
    } else {
      out += 'exports[\'default\'] = '
  ...
  async ExportNamedDeclaration(node, code, _id) {
    const defines = [];
    const declarations = [];
    const exports = [];
        ...
        case 'VariableDeclaration': {
          ...
          for (const {id, init} of decl.declarations) {
            ...
            if (id.type === 'ObjectPattern') {
              const name = `__bpkg_export_${_id}_${tmp++}__`;
    ...
    if (source) {
      const name = `__bpkg_export_${_id}__`;
      ...
      for (const [value, key] of exports) {
        if (value === 'default') {
          out += `__bpkg_declare__(exports, ${string(key)}, function() {\n`
