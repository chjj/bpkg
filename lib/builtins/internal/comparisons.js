...
SlowMap.prototype._indexOf = function(key) {
...
SlowMap.prototype.has = function(key) {
  return this._indexOf(key) >= 0;
};

SlowMap.prototype.get = function(key) {
  var i = this._indexOf(key);
  return i >= 0 ? this._items[i].value : undefined;
};

SlowMap.prototype.set = function(key, value) {
  var i = this._indexOf(key);
  if (i >= 0) {
...
SlowMap.prototype['delete'] = function(key) {
  var i = this._indexOf(key);
  if (i >= 0) {
    if (i !== this._items.length - 1)
      this._items[i] = this._items.pop();
    else
      this._items.pop();
    this.size--;
  }
