...
class Bundle {
  ...
  async getModule(path) {
    ...
    const module = this.moduleMap.get(path);

    if (this.bundle.loose && module.isModule() && !module.opened)
      throw new Error(`Recursive import: ${module.pkg.name}${module.subpath}`);
  ...
  async analyzeModule(path) {
    ...
    if (!this.analyzeMap.has(path)) {
      ...
      this.log('Analyzing module: %s%s.',
               module.pkg.name,
               module.subpath);

      this.analyzeMap.set(path, module);

      await module.analyze();
    }

    return this.analyzeMap.get(path);
  }
  ...
  async bundle(input, output) {
    ...
    if (input === root) {
      ...
      if (this.entryType != null)
        ...
        isImport = pkg.isModule();

      input = await pkg.resolver.resolve(input, isImport);
    }
  ...
  async _bundle(input, output) {
    ...
    for (const name of this.requires) {
      ...
      if (module.isModule())
        throw new Error(`Cannot require() ES module: ${string(name)}`);
    ...
    for (const name of this.imports) {
      ...
      const module_ = await this.getModule(path);

      if (module_.isAsync())
        throw new Error(`Cannot import async module: ${string(name)}`);

      requires.push(module_.id);
    }
    ...
    if (this.env === 'node' &&
      ...
      if (this.env === 'browser') {
        out = await template('browser.js', {
          ...
          hassetflag: this.hasSetFlag,
          hasassign: this.hasAssign,
          hasproxy: this.hasProxy,
          hasexpose: this.hasExpose,
          importdefault: this.hasImportDefault,
          importstar: this.hasImportStar,
          exportstar: this.hasExportStar,
