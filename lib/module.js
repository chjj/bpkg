...
const ESM_WRAPPER = [
  'function(__exports, __meta) {',
  '}'
];

const CJS_BROWSER = [
  'function(exports, require, module, __filename, __dirname, __module) {',
  '}'
];
...
class Module {
  ...
  async makeWasm(path) {
    ...
    if (imports.length > 0) {
      ...
      for (const specifier of modules) {
        const call = await this.linker.require(specifier, true);
  ...
  async compile(link = true) {
    ...
    if (this.format === 'wasm')
      return this.makeWasm(this.filename);
  ...
  wrap(code) {
    ...
    const browser = this.bundle.env === 'browser';

    if (this.format === 'module') {
      const wrapper = browser ? ESM_BROWSER : ESM_WRAPPER;
    ...
    const wrapper = browser ? CJS_BROWSER : CJS_WRAPPER;
