...
class ESM {
  ...
  makeRequire(specifier, attrs = null) {
    if (attrs != null)
      return `__esm_import__(${string(specifier)}, ${attrs})`;
  ...
  async analyzeModule(specifier) {
    ...
    if (module.esm.hasTLA)
      this.hasTLA = module.esm.hasTLA;
    else if (!module.analyzed)
      module.esm.parents.push(this);
  ...
  async transform(root, code) {
    ...
    if (this.bundle.target !== 'esm' && esm.hasTLA(root)) {
      const file = this.module.relative;
  ...
  async ExportAllDeclaration(node, code) {
    const source = node.source.value;
    const attrs = sliceAttrs(node, code);
    ...
    if (node.exported) {
      // export * as foo from 'module';
      const exportedName = getName(node.exported);
      const expr = this.makeRequire(source, attrs);
    ...
    if (snapshot) {
      ...
      out += this.makeVar(name, this.makeRequire(source, attrs));
      ...
      out += this.makeExportStar(this.makeRequire(source, attrs));
    }
  ...
  async ImportDeclaration(node, code) {
    ...
    if (node.specifiers.length === 0) {
      // import 'module';
      out += this.makeRequire(source, attrs) + ';\n';
      return [node, out];
    }
      ...
      switch (type) {
        ...
        case 'ImportDefaultSpecifier': {
          ...
          if (this.bundle.loose || this.consts.has(local.name)) {
            if (this.bundle.es2015 && expr[expr.length - 1] === ']')
        ...
        case 'ImportSpecifier': {
          ...
          let es2015 = this.bundle.es2015 | 0;
          let expr;

          if (importedName === 'default') {
            expr = await this.makeDefault(name, source);
            es2015 &= (expr[expr.length - 1] === ']') | 0;
          } else {
            expr = `${name}${dot(importedName)}`;
          }

          if (this.bundle.loose || this.consts.has(local.name)) {
            if (es2015)
              imports.push([importedName, local.name]);
            else
              out += this.makeVar(local.name, expr);
          } else {
    ...
    if (imports.length > 0) {
      ...
      let len = 12 + name.length;

      for (const [imported, local] of imports) {
        if (!isIdentifer(imported))
          len += imported.length + local.length + 6;
        else if (imported !== local)
          len += imported.length + local.length + 4;
        else
          len += imported.length + 2;
      }

      if (len <= 80) {
        for (const [imported, local] of imports) {
          if (!isIdentifer(imported))
            tmp.push(`${string(imported)}: ${local}`);
          else if (imported !== local)
            tmp.push(`${imported}: ${local}`);
          else
            tmp.push(`${imported}`);
        }
