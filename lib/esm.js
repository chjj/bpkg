...
class ESM {
  ...
  async isRealImport(location) {
    ...
    if (this.resolver.isExternal(location, true))
      return true;
    ...
    if (this.bundle.ignoreMissing) {
      try {
        await this.resolver.resolve(location, true);
      } catch (e) {
        return true;
      }
    }
  ...
  makeRequire(location) {
    return `__esm_import__(${string(location)})`;
  }
  ...
  async getExports(location) {
    ...
    try {
      path = await this.resolver.resolve(location, true);
    } catch (e) {
      return new Set();
    }
  ...
  async transform(code) {
    ...
    if (this.bundle.target !== 'esm') {
      let tmp = this.makeStrict();

      if (this.bundle.multi) {
        if (this.hasMeta)
          tmp += this.makeMeta();

        tmp += '(function(__exports) {';
        tmp += '\n';
        tmp += out;
        tmp += '\n';
        tmp += '}).call(void 0, exports);';
      } else {
        tmp += out;
      }
  ...
  async ImportExpression(node, code) {
    ...
    const value = this.makeRequire(arg.value);
