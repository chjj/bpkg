...
const {getString, isString, string} = utils;
...
class Linker {
  ...
  isBufferCheck(node) {
    ...
    if (!this.globalRefs.has(callee.object))
      return false;
  ...
  isProcessCwd(node) {
    ...
    if (callee.object.name !== 'process')
      return false;

    if (callee.property.name !== 'cwd')
      return false;
  ...
  async AwaitExpression(node, code) {
    ...
    return [node, await this.require(arg, importTypes.IMPORT)];
  }
  ...
  async ImportExpression(node, code) {
    ...
    return [node, await this.require(arg, importTypes.IMPORT_EXPR)];
  }
