...
class ESM {
  ...
  async CallExpression(node, code) {
    if (this.isImport(node)) {
      ...
      if (arg.type !== 'Literal' || typeof arg.value !== 'string') {
        ...
        value = await this.makeRequire(arg.value);
      }
  ...
  async ExportAllDeclaration(node, code) {
    ...
    out += this.makeVar(name, await this.makeRequire(source));
  ...
  async ImportDeclaration(node, code) {
    ...
    if (node.specifiers.length === 0) {
      // import 'module';
      out += (await this.makeRequire(source)) + ';\n';
      return [node, out];
    }
