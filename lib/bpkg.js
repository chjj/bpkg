...
class Bundle {
  ...
  async getExports(path) {
    return (await this.getModule(path, null)).exports;
  }
  ...
  async _build() {
    ...
    if (this.env === 'node'
        ...
        && requires.length === 0
        && !this.hasMeta
        && (!this.esm || module.exports.size === 0)) {
      if (module.hashbang)
        out += module.hashbang + '\n';
      ...
      if (this.env === 'browser') {
        out = await template('browser.js', {
          header,
          esm: this.esm,
          prefix: this.esm ? 'var __browser_require__ = ' : ';',
          timers,
          microtask,
          process,
          buffer,
...
class Module {
  constructor(bundle, path, rawCode) {
    ...
    this.esm = false;
    this.meta = false;
    this.exportNames = new Set(['exports']);
  ...
  warning(log, ...args) {
    this.bundle.log(`Warning (%s): ${log}`, relative(CWD, this.path), ...args);
  }
  ...
  makeExports() {
    ...
    for (const key of this.exports) {
      ...
      if (key === '') {
        out += `export default ${expr};\n`;
        continue;
      }

      if (key === 'default') {
        out += `export default ${expr}['default'];\n`;
        continue;
      }

      out += `export const ${key} = ${expr}.${key};\n`;
    }
  ...
  async transform(code) {
    ...
    if (this.bundle.multi) {
      if (this.bundle.esm == null)
        this.bundle.esm = true;
    } else {
      if (this.bundle.esm == null)
        this.bundle.esm = type === 'module';
    }
    ...
    for (const node of acorn.walk(root)) {
      ...
      switch (node.type) {
        ...
        case 'ExpressionStatement':
          filter = this.ExpressionStatement;
          break;
        case 'VariableDeclaration':
          filter = this.VariableDeclaration;
          break;
        case 'MetaProperty':
  ...
  async require(location) {
    ...
    if (this.bundle.multi) {
      if (location.includes('/') || location.includes('\\'))
        location = await this.bundle.rewrite(this, location);
  ...
  async getExports(location) {
    ...
    return new Set(Object.keys(await this.resolve.require(path)));
  }
  ...
  isNakedExports(node) {
    ...
    if (!this.exportNames.has(node.name))
      return false;
  ...
  isExports(node) {
    ...
    // module.exports
    // module['exports']
    return this.isNakedExports(node)
  ...
  isExportsAssign(node) {
    ...
    if (!this.isNakedExports(member.object))
      return false;
  ...
  isModuleExportsAssign(node) {
    ...
    return this.isModuleExports(expr.left);
  }

  isExportsAssignExpr(expr) {
    // e.g.
    // exports = foo
    // exports = {}
    // module.exports = foo
    ...
    if (!this.isExports(expr.left))
      return null;

    // Descend into the right subtree.
    const right = this.isExportsAssignExpr(expr.right);
  ...
  isExportsReassign(node) {
    ...
    const right = this.isExportsAssignExpr(node.expression);
  ...
  isExportsRenameExpr(node) {
    // e.g.
    // x = exports;
    // x = exports = {};
    // x = exports = foo;
    // x = exports = module.exports = {};
    // x = exports = module.exports = foo;
    if (node.type !== 'ExpressionStatement')
      return null;
    ...
    if (this.isExportsAssignExpr(expr.right))
      return expr.left.name;

    if (this.isExports(expr.right))
      return expr.left.name;

    return null;
  }

  isExportsRenameDecl(node) {
    // e.g.
    // const x = exports;
    // const x = exports = {};
    // const x = exports = foo;
    // const x = exports = module.exports = {};
    // const x = exports = module.exports = foo;
    ...
    for (const decl of node.declarations) {
      ...
      const right = this.isExportsAssignExpr(decl.init);
  ...
  async ExportAllDeclaration(node, code, id) {
    ...
    for (const key of (await this.getExports(source))) {
      if (key === '' || key === 'default')
        continue;
  ...
  async ExpressionStatement(node, code, id) {
    if (!this.bundle.esm)
      return null;

    const name = this.isExportsReassign(node)
              || this.isExportsRenameExpr(node);
    ...
    if (this.isExportsAssign(node)) {
      const prop = node.expression.left.property;

      if (prop.type === 'Identifier')
        this.exports.add(prop.name);
      else
        this.exports.add(prop.value);

      this.exports.delete('default');
      this.exports.delete('');
      this.exports.add('');
    ...
    if (this.isModuleExportsAssign(node)) {
      this.exports.delete('default');
      this.exports.add('');
      return null;
    }
  ...
  async VariableDeclaration(node, code, id) {
    if (!this.bundle.esm)
      return null;

    const names = this.isExportsRenameDecl(node);
