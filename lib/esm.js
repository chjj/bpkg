...
class ESM {
  ...
  async analyze(root) {
    ...
    for (const [name, desc] of exports) {
      ...
      if (this.resolver.isExternal(source, true))
        continue;
  ...
  async ExportNamedDeclaration(node, code) {
      ...
      switch (decl.type) {
        case 'VariableDeclaration': {
          ...
          for (const {id, init} of decl.declarations) {
            ...
            if (init)
              expr = code.substring(init.start, init.end);

            if (id.type === 'Identifier') {
              declarations.push([id.name, expr, kind]);
              continue;
            }
            ...
            if (id.type === 'ObjectPattern') {
              ...
              for (const {key, value} of id.properties) {
                ...
                declarations.push([value.name, `${name}[${prop}]`, kind]);
              }
            ...
            if (id.type === 'ArrayPattern') {
              ...
              for (let i = 0; i < id.elements.length; i++) {
                ...
                if (el.type !== 'Identifier')
                  throw new Error(`Unexpected token: ${el.type}.`);

                declarations.push([el.name, `${name}[${i}]`, kind]);
              }
        ...
        case 'ClassDeclaration': {
          ...
          declarations.push([id.name, code.substring(start, end), 'func']);
    ...
    if (source != null) {
      ...
      for (const [key, value, kind] of declarations) {
        if (kind === 'func') {
          out += value + '\n';
        ...
        out += this.makeVar(key, value, kind);
