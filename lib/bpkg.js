...
class Module {
  ...
  async transform(code) {
    ...
    // One pass to replace fs.readFile calls.
    if (!this.bundle.multi) {
      for (const node of acorn.walk(root)) {
        if (node.type !== 'CallExpression')
          continue;

        const value = await this.CallExpressionForRead(node, code);

        if (value != null) {
          out += code.substring(offset, node.start);
          out += value;
          offset = node.end;
        }
      }
      ...
      if (offset !== 0) {
        ...
        [code, out, offset] = [out, '', 0];
        root = parseCode(code, type, this.path);
      }
    }

    // Second pass for simpler analysis.
    for (const node of acorn.walk(root)) {
      let filter;
      ...
      const result = await filter.call(this, node, code);
  ...
  isReadFile(node) {
    ...
    if (callee.property.name === 'readFileSync') {
      if (node.arguments.length < 1
    ...
    if (callee.property.name === 'readJSONSync') {
      if (node.arguments.length !== 1)
        return false;
      return true;
    }
  ...
  async readFile(node, code) {
    ...
    if (method === 'readFileSync') {
      if (node.arguments.length > 1)
        encoding = node.arguments[1].value;
    }
  ...
  evalExpression(expr) {
    ...
    const {resolve} = createRequireFromPath(this.path);
    ...
    const sandbox = {
      ...
      __dirname: this.dir,
      __url: pathToFileURL(this.path).href,
      url,
      URL,
      fileURLToPath,
      pathToFileURL,
    ...
    expr = expr.replace(/import\.meta\.url/g, '__url');
  ...
  async CallExpressionForRead(node, code) {
  ...
  async ImportDeclaration(node, code) {
    ...
    out += this.makeVar(name, await this.require(source));
