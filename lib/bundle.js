...
class Bundle {
  ...
  async writeBuild(output) {
    ...
    const [code, cmk] = await this.createBuild(output);
    ...
    if (cmk) {
      await fs.mkdir(join(output, 'node-cmk'));
      ...
      for (const name of ['bin',
                          ...
                          'src',
                          'package.json']) {
        await fs.copy(join(cmk, name), join(output, 'node-cmk', name));
      }
    }
  }
  ...
  async createBuild(output) {
    ...
    for (const [pkg] of installs) {
      if (pkg.install.includes('node-gyp'))
        gyp = true;
      else if (pkg.install.includes('node-cmk'))
        cmk = true;
    }

    if (cmk) {
      try {
        cmkPath = await this.resolve.directory('node-cmk');
      } catch (e) {
        cmk = false;
      }
    }
