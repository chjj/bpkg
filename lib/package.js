...
class Package {
  constructor(bundle, root) {
    ...
    this.resolver = bundle.createResolver(this.root, true);
  }
  ...
  async init() {
    ...
    const json = clone(await this.resolver.readPackageJson(root));
    const deps = [];
    const bin = [];
    const man = [];
  ...
  isModule() {
    ...
    if (this.bundle.target === 'esm') {
      if (this.json.exports) {
        const main = this.json.exports['.'];
