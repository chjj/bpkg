...
const {dot, getName, string} = utils;
    ...
    for (const node of root.body) {
      switch (node.type) {
        ...
        case 'ExportDefaultDeclaration': {
          ...
          const name = getObjectName(decl);

          this.exports.add('default');

          // Always a constExport unless it's an unsafe import.
          // Maybe keep track of safe imports separately.
          if (!name || this.consts.has(name))
