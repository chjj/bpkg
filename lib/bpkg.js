...
class Package {
  ...
  init() {
    ...
    if (pkg.browserify == null)
      pkg.browserify = Object.create(null);

    if (typeof pkg.browserify !== 'object')
      throw new Error('Invalid package.browserify.');

    if (pkg.browserify.transform == null)
      pkg.browserify.transform = [];

    if (typeof pkg.browserify.transform === 'string')
      pkg.browserify.transform = [pkg.browserify.transform];

    if (!Array.isArray(pkg.browserify.transform))
      throw new Error('Invalid package.browserify.transform.');

    const {transform} = pkg.browserify;
    ...
    this.transforms = pkg.browserify.transform;
    this.deps = deps;
  ...
  async _transform(file, code, transform) {
    ...
    const stream = require(path)(file, options);
