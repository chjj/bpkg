...
class Linker {
  ...
  async tryBinding(location) {
    ...
    const gypFile = resolve(this.module.root, 'binding.gyp');

    if (!await fs.exists(gypFile))
      return null;

    this.log('Rebuilding binding: %s.', this.module.root);

    await gypRebuild(this.module.root);
  ...
  async CallExpression(node, code) {
    ...
    if (this.isBindings(node, code)) {
      ...
      path = unix(relative(this.module.dir, path));
...
async function gypRebuild(root) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      cp.execFile('node-gyp', ['rebuild'], { cwd: root }, cb);
    } catch (e) {
      reject(e);
    }
  });
