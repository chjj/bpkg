...
class Bundle {
  constructor(options) {
    ...
    this.tarball = null;
    this.version = '0.0.0';
    this.path = join(process.cwd(), 'package.json');
    this.root = process.cwd();
    this.resolve = this.resolver(process.cwd());
  ...
  init(options) {
    ...
    if (options.code != null) {
      assert(typeof options.code === 'string');

      this.code = options.code;

      if (typeof options.input !== 'string')
        this.input = resolve(DUMMY);
    }
  ...
  createModule(path, code) {
    return new Module(this, path, code);
  }
  ...
  async initPkg() {
    ...
    if (this.code == null || basename(this.input) !== DUMMY)
  ...
  async build() {
    ...
    this.log({
      ...
      ignoreBindings: this.ignoreBindings,
...
class Module {
  constructor(bundle, path, rawCode) {
    assert(bundle instanceof Bundle);
    assert(typeof path === 'string');
    assert(rawCode == null || typeof rawCode === 'string');
  ...
  warning(log, ...args) {
    this.log(`Warning (%s): ${log}`,
      relative(process.cwd(), this.path),
      ...args);
  }
  ...
  async open() {
    this.code = await this.compile(true);
    return this;
  }
  ...
  walk(root) {
    return acorn.walk.collect(root);
  }

  async replace(code, ctx, desc) {
    ...
    for (const node of this.walk(root)) {
      const filter = desc[node.type];

      if (!filter)
        continue;

      const result = await filter.call(ctx, node, code);
  ...
  async makeJSON(path) {
    ...
    const json = JSON.parse(stripBOM(code));
  ...
  async compile(link = true) {
    ...
    if (code == null)
      code = await fs.readFile(this.path, 'utf8');
  ...
  prepare(text) {
    ...
    if (this.bundle.esm == null) {
      this.bundle.esm = this.bundle.multi
  ...
  async transform(code) {
    ...
    const [, out] = await this.replace(code, this, {
