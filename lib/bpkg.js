...
class Bundler {
  ...
  bundle(path, output) {
    ...
    output = output ? resolve(CWD, output) : null;
    ...
    if (this.multi) {
      ...
      if (output.endsWith('.tar.gz')) {
        ...
        output = resolve(PREFIX, `${name}-${Date.now()}`, name);
      }

      const pkg = this.pkg(path);
...
class Package {
  constructor(bundler, root) {
    ...
    this.deps = [];
    this.license = null;
    this.require = Module_.createRequireFromPath(this.root);
    this.resolve = this.require.resolve.bind(this.require);
  }
...
class Module {
  ...
  requirify() {
    ...
    const pathRequire = Module_.createRequireFromPath(path);
