...
class Bundler {
  ...
  resolver(path) {
    return resolver(path, this.extensions);
  }
...
class Package {
  constructor(bundler, root) {
    ...
    this.version = '0.0.0';
    this.main = null;
    this.bin = Object.create(null);
    this.browser = Object.create(null);
    this.deps = [];
    this.license = null;
    this.resolver = bundler.resolver(this.root);
  ...
  async init() {
    ...
    for (const key of Object.keys(pkg.browser)) {
      const value = pkg.browser[key];

      delete pkg.browser[key];

      if (value === false) {
        const empty = './' + unix(relative(root, builtins._empty));
        pkg.browser[await normalize(key)] = empty;
        continue;
      }

      pkg.browser[await normalize(key)] = await normalize(value);
    }
  ...
  async normalize(location) {
    if (typeof location !== 'string')
      throw new Error('Invalid package field.');
    ...
    const resolved = await this.resolve(unix(path));

    return './' + unix(relative(this.root, resolved));
  }

  async _resolve(location) {
    const path = await this.resolver(location);
    return this.alias(path);
  }
  ...
  alias(path) {
    ...
    if (!isAbsolute(path)) {
      if (this.browser[path])
        return resolve(this.root, this.browser[path]);
      return path;
    }

    const file = './' + unix(relative(this.root, path));

    if (this.browser[file])
      return resolve(this.root, this.browser[file]);
...
class Module {
  constructor(bundler, path, rawCode) {
    ...
    this.code = '';
    this.wrapped = '';
    this.resolver = bundler.resolver(path);
    this.resolve = this._resolve.bind(this);
  }
  ...
  async init() {
    ...
    // Might be a pointer to a browser file,
    // in which case, we need to re-initialize.
    this.path = this.pkg.alias(this.path);
    this.dir = dirname(this.path);
    this.name = basename(this.path);
    this.ext = extname(this.path);
    this.type = this.getType();
    this.resolver = this.bundler.resolver(this.path);
    this.resolve = this._resolve.bind(this);
