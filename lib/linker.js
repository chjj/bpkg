...
class Linker {
  ...
  _makeRequire(id, specifier) {
    if (this.bundle.env === 'browser')
      return `__browser_require__(${id} /* ${string(specifier)} */, __module)`;

    return `__node_require__(${id} /* ${string(specifier)} */)`;
  }

  makeRequire(id, specifier) {
    assert(typeof id === 'number');
    assert(typeof specifier === 'string');
    assert(!this.bundle.multi);

    if (this.bundle.target === 'esm')
      return '(await ' + this._makeRequire(id, specifier) + ')';

    return this._makeRequire(id, specifier);
  }

  async makeRawRequire(specifier, isImport) {
    assert(typeof specifier === 'string');
  ...
  async require(specifier, isImport) {
    assert(typeof specifier === 'string');
    assert(typeof isImport === 'boolean');
    ...
    if (this.bundle.multi) {
      ...
      if (this.resolver.isLocal(specifier)) {
        if (extname(specifier) !== '')
          specifier = await this.bundle.rewrite(this.module, specifier);
      }

      if (isImport && !(await this.resolver.requirable(specifier)))
        throw new Error(`Cannot import ${specifier} with require.`);

      return this.makeRawRequire(specifier, false);
    }

    if (this.resolver.isExternal(specifier, isImport)) {
      this.warning('ignoring external module: %s.', specifier);
      return this.makeRawRequire(specifier, isImport);
    }
    ...
    if (isAbsolute(path)) {
      ...
      return this.makeRequire(id, specifier);
    }
