...
// if __WASM__
function __browser_unbase64__(str) {
  var data = atob(str);
...
// if __WASMSYNC__
function __browser_compile_sync__(name, raw, imports) {
  if (typeof WebAssembly !== 'object' || WebAssembly === null)
    throw new Error('WebAssembly not supported.');

  var source = __browser_unbase64__(raw);
...
// if __WASMASYNC__
async function __browser_compile_async__(name, raw, imports) {
  if (typeof WebAssembly !== 'object' || WebAssembly === null)
    throw new Error('WebAssembly not supported.');
