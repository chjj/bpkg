...
class Bundler {
  ...
  async pluginCompile(module, code) {
...
class Package {
  ...
  async init() {
    ...
    for (const name of ['LICENSE',
      ...
      if (await isFile(path)) {
        ...
        this.bundler.hasLicense = !this.isBuiltin();
        break;
      }
    }
  ...
  async _copy(dest) {
    ...
    for (const [file, stat] of (await this.traverse())) {
      ...
      code = await this.bundler.pluginCompile(module, code);
...
class Module {
  ...
  async compile() {
    ...
    if (this.type === 'bin' || this.type === 'lib') {
      code = await this.bundler.pluginCompile(this, code);
  ...
  async require(location) {
    ...
    if (isAbsolute(path)) {
      const module = await this.bundler.module(path);
      return this.makeRequire(module.id);
    }
