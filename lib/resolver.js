...
class Resolver {
  ...
  async resolvePath(x, y) {
    ...
    x = await this.redirect(x, y, false);
  ...
  async requireX(x, y) {
    ...
    if (x[0] === '#') {
      const a = await this.loadPackageImports(x, y);
  ...
  async imports(location) {
    const [path] = await this.resolveImport(location);
    return this.importFunc(path);
  }

  async resolveImport(location) {
    const [path, format] = await this.resolveImportPath(location, this.root);
  ...
  async resolveImportPath(x, y) {
    ...
    // Possible redirection.
    x = await this.redirect(x, y, true);

    if (x == null) {
      this.cache[xy] = [null, null];
      return [null, null];
    }
    ...
    // Resolve path.
    const c = this._conditions(true);
    const z = await this.esmResolve(x, y, c);
  ...
  async esmResolve(specifier, parentDir, conditions) {
    ...
    if (isFileURL(specifier)) {
      ...
      resolved = await this.packageResolve(specifier, parentDir, conditions);
    }
  ...
  async packageExportsResolve(packageDir, subpath, exports, conditions) {
    ...
    if (hasDots === true) {
      ...
      // 3.2
      const resolved = await this.packageImportsExportsResolve(matchKey,
                                                               exports,
                                                               packageDir,
                                                               false,
                                                               conditions);
  ...
  async packageImportsResolve(specifier, parentDir, conditions) {
    ...
    if (packageDir !== null) {
      ...
      // 4.2
      if (pjson && isObject(pjson.imports)) {
        // 4.2.1
  ...
  async packageTargetResolve(packageDir,
    ...
    if (isString(target)) {
      ...
      // 1.4
      assert(packageDir.includes(resolvedTarget));
    ...
    // 2
    if (isObject(target)) {
      const keys = Object.getOwnPropertyNames(target); // exports?
  ...
  async esmFileFormat(file) {
    ...
    const packageDir = await this.lookupPackageScope(file);
    ...
    if (pjson && (pjson.type === 'module' || pjson.type === 'commonjs'))
