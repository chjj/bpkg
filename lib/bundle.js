  ...
  init(options) {
    ...
    if (typeof options.target === 'string') {
      assert(options.target === 'cjs' ||
             options.target === 'umd' ||
             options.target === 'esm' ||
             options.target === 'esm-sync');

      if (options.target === 'esm-sync')
        this.target = 'esm';
      else
        this.target = options.target;

      this.isAsync = options.target === 'esm';
    }
    ...
    if (this.env === 'node' &&
      ...
      if (this.env === 'browser') {
        out = await template('browser.js', {
          ...
          console,
          meta: this.hasMeta,
          wasmsync: this.hasWasm && !this.isAsync,
          wasmasync: this.hasWasm && this.isAsync,
