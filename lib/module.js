...
class Module {
  ...
  async init() {
    this.root = await Resolver.mustFindRoot(this.dirname);
  ...
  async makeWasm() {
    ...
    const target = this.isAsync() ? 'async' : 'sync';
  ...
  async _readCode() {
    ...
    [hash, code] = prepareCode(code);

    if (this.bundle.env === 'browser' && this.id === 0 && !hash)
      [info, code] = stripUserScriptInfo(code);

    code = this.resolver.maybeStrip(this.filename, code);
    code = await this.bundle.compile(this, code);

    const root = this.parse(code);

    return [root, code, hash, info];
  }
  ...
  async compile() {
    ...
    let [root, code, hash, info] = await this.readCode();
  ...
  isAsync() {
    return this.esm.hasTLA;
  }

  getFlag() {
    if (this.isAsync()) {
      // Only possible with format == 'module'
      //                and target === 'esm'.
      return 2;
    }
    return this.isModule() ? 1 : 0;
  }
...
function stripUserScriptInfo(code) {
  const rx0 = /^\s*(\/\*|\/\/)[ \t]*==UserScript==[ \t]*\n/;
  const rx1 = /\n\/\/[ \t]*==\/UserScript==[ \t]*\n\s*/;
  const rx2 = /\s\s*==\/UserScript==\s*\*\/\s*/;
  const m1 = rx0.exec(code);

  if (!m1)
    return ['', code];

  const m2 = m1[1] === '//' ? rx1.exec(code) : rx2.exec(code);

  if (!m2)
    return ['', code];

  const index = m2.index + m2[0].length;
